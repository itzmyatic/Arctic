...
let handler = async (m, { conn, args, isPrems, isOwner }) => {
  if (!args || !args[0] || !args[0].startsWith('http')) throw 'Uhm... urlnya mana?'
  //if (!args || !args[0]) throw 'Uhm... urlnya mana?'
  let chat = await db.users.get[m.chat]
  const isY = /y(es)/gi.test(args[1])
  const { thumbnail, audio: _video, title } = await youtubedl(args[0]);
