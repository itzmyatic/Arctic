 ...
 */
const { getBinaryNodeChild, getBinaryNodeChildren, areJidsSameUser } = (await import('@whiskeysockets/baileys')).default
let handler = async (m, { conn, text, participants, isROwner, isOwner }) => {
    // bot utama add hanya bisa digunakan oleh owner yang telah didefinisikan di config.js
    if (areJidsSameUser(Connection.conn.user.id, conn.user.id) && !isROwner)
